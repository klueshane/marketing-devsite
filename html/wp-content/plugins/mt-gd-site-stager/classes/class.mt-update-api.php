...
class MT_Update_API {
	 ...
	 * @var string
	 */
	private $_slug = 'mt-gd-site-stager';
	...
	protected function get_args() {
		...
		return array(
				'headers'   => array(
					...
					'X-Plugin-Version'        => get_option( 'mtss_setup_version' ),
	...
	protected function make_call( $method, $args = array(), $verb = 'GET' ) {
		...
		while ( $retries <= $max_retries ) {
			...
			if ( 'GET' === $verb ) {
				...
				$_args['body'] = $args;
				add_filter( 'https_ssl_verify', '__return_false' );
				$result = wp_remote_post( MT_UPDATE_URL . $method, $_args );
				remove_filter( 'https_ssl_verify', '__return_false' );
			}
		...
		do_action( 'mt_update_api_debug_request', MT_UPDATE_URL . $method, $this->get_args() );
