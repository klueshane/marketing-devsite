...
$_mt_upgrade_url = 'https://wpqs.secureserver.net/v1/';  // trailing slash is important
$_mt_api = new MediaTemple_API( $_mt_upgrade_url );
$_mt_upgrader = new MediaTemple_Upgrader();
$_mt_upgrader->set_api( $_mt_api );

/**
 * Upgrade plugins from a third party system
 */
class MediaTemple_Upgrader {
	...
	protected function get_api_data() {
		$api_data = get_site_transient( 'mt_theme_update_api_data' );
		if ( empty( $api_data ) ) {
			...
			set_site_transient( 'mt_theme_update_api_data', $api_data, 60 * 60 * 6 );
		}
		return $api_data;
	}
