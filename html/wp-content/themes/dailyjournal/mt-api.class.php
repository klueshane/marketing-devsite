...
class MediaTemple_API {
	...
	protected function make_call( $method, $args = array(), $verb = 'GET' ) {
		...
		if ( !in_array( $verb, array( 'GET', 'POST' ) ) ) {
			return new WP_Error( 'mt_api_bad_verb', sprintf( __( 'Unknown verb: %s. Try GET or POST', 'mt-journal' ), $verb ) );
		}
		while ( $retries <= $max_retries ) {
			...
			if ( is_wp_error( $result ) ) {
				...
				sleep( apply_filters( 'mt_api_retry_delay', 1 ) );
				continue;
			} else {
				break;
			}
		...
		if ( !is_wp_error( $result ) && '200' != $result['response']['code'] ) {
			return new WP_Error( 'mt_api_bad_status', sprintf( __( 'API returned bad status: %d: %s', 'mt-journal' ), $result['response']['code'], $result['response']['message'] ) );
		}
